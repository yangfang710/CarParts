...
<style type="text/css">
  ...
  <body>
    ...
    <form action="<c:url value='/admin/AdminCategoryServlet'/>" method="post" onsubmit="return checkForm()">
    	...
    	分类名称：<input type="text" name="catName" value="${child.catName }" id="catName"/><br/>
    	一级分类：<select name="pid" id="pid">
    		<option value="">===选择1级分类===</option>
<c:forEach items="${parents }" var="parent">
    		<option value="${parent.cid }" <c:if test="${parent.cid eq child.parent.cid }">selected="selected"</c:if> >${parent.catName }</option>
</c:forEach>
    	</select><br/>
