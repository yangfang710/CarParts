...
public class AdminAddCarServlet extends HttpServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		Car car = CommonUtils.toBean(map, Car.class);// 把Map中大部分数据封装到Book对象中
		 ...
		 * 保存图片： 1. 获取真实路径
		 */
		String savepath = this.getServletContext().getRealPath("/car_img");
		/*
		 * 2. 创建目标文件
		 */
		...
		// 把图片的路径设置给book对象
		car.setImage_w("car_img/" + filename);
		 ...
		 * 保存图片： 1. 获取真实路径
		 */
		savepath = this.getServletContext().getRealPath("/car_img");
		/*
		 * 2. 创建目标文件
		 */
		...
		// 把图片的路径设置给book对象
		car.setImage_b("car_img/" + filename);

		// 调用service完成保存
		car.setCarId(CommonUtils.uuid());
		CarService carService = new CarService();
		carService.add(car);

		// 保存成功信息转发到msg.jsp
		request.setAttribute("msg", "添加配件成功！");
	...
	private void error(String msg, HttpServletRequest request,
		...
		request.getRequestDispatcher("/adminjsps/admin/car/add.jsp").forward(
				request, response);
	}
}
